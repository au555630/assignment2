...
```{r}
demo_train= read.csv("C:/Users/Viki/Documents/egyetem/exp.met/port1/assignment1/demo_train(1).csv")
LU_train= read.csv("C:/Users/Viki/Documents/egyetem/exp.met/port1/assignment1/LU_train.csv")
token_train= read.csv("C:/Users/Viki/Documents/egyetem/exp.met/port1/assignment1/token_train.csv")
```
...
demo_train = plyr::rename(demo_train, c("Visit" = "visit"))
LU_train = plyr::rename(LU_train, c("VISIT" = "visit"))
token_train = plyr::rename(token_train, c("VISIT" = "visit"))
...
demo_train$id <- gsub("\\.", "", demo_train$id, perl = TRUE)
LU_train$SUBJ <- gsub("\\.", "", LU_train$SUBJ, perl = TRUE)
token_train$SUBJ <- gsub("\\.", "", token_train$SUBJ, perl = TRUE)
...
LU_train = plyr::rename(LU_train, c("SUBJ" = "id"))
token_train = plyr::rename(token_train, c("SUBJ" = "id"))
demo_train = plyr::rename(demo_train, c("MullenRaw" = "nonverbal_IQ"))
demo_train = plyr::rename(demo_train, c("ExpressiveLangRaw" = "verbal_IQ"))

library(tidyverse)
demo_train1= select(demo_train, id, visit, Ethnicity, Diagnosis, Gender, Age, ADOS, nonverbal_IQ, verbal_IQ)
LU_train1= select(LU_train, id, visit, MOT_MLU, MOT_LUstd, CHI_MLU, CHI_LUstd)
token_train1= select(token_train, id, visit, types_MOT, types_CHI, tokens_MOT, tokens_CHI)
```
...
```{r}
df= merge(demo_train1, LU_train1)
#keepall= merge (demo_train1, LU_train1, all= T)
#problem: rows disappear because they only exist in one dataset. For ex. there's AA in demo_train but not in LU_train.
#I would discard the data, because we only have the demographic data for them, and not everything else we are interested in. Maybe they arrived to the visit but something happened and they didn't do the interactions?

port1=merge(df, token_train1)
```
...
```{r}
write.csv(final, file = "port1_data.csv", row.names = F)
...
#1.
#my version 
one = na.omit(final) #get out na-s
uno_means= summarise(group_by(one, id), save_mean=mean(CHI_MLU)) #mean of CHI_MLU belonging to the same id
uno=filter(uno_means, save_mean > 2.7) # get out all the rows where the mean is > 2.7

#trying out the pipe version
# take "one", group it by id, make the mean of CHI_MLU for each id. get out the rows where meanMLU is > 2.7
one = na.omit(final) 
more_across_all= one %>% 
      group_by(id) %>%
...
#2.
subset_two= subset(final, final$visit == "1", select= c(id,CHI_MLU))
two= unique(filter(subset_two, subset_two$CHI_MLU < 1.5))


#3.
#my code
#get out all rows containing NAs (aka not completed trials)
#count how many rows are there (this would be the tally function)
#get out the ones where there are less than 6 rows
tres_na= na.omit(final)
tres= summarise(group_by(tres_na, id), save= length(id))
tres_missing= filter(tres, save < 6)

#trying out Celine's code with pipes
three=na.omit(final)
not_all= three %>%
    group_by(id) %>% 
    tally() %>% #counts rows
    filter(n < 6)
#get out all NAs, count the rows for each participant, select the participants where there are rows missing.
...
```{r}
#1.
most_6= subset(final, final$visit== 6, select=c(id:verbal_IQ))
most= arrange(most_6, desc(tokens_CHI))

#2.
least_1= subset(final, final$visit== 1, select=c(id:verbal_IQ))
least= arrange(least_1, tokens_CHI)
```
...
only_1=filter(final, Diagnosis == "ASD")
only= select(only_1, CHI_MLU, MOT_MLU, tokens_CHI, tokens_MOT)

what_happens= select(only_1, CHI_MLU, MOT_MLU, tokens_CHI, tokens_MOT, tokens_MOT)
# R is smart and doesn't put the column there twice
```
...
```{r}
#1.
#I'm confused about what the task is? But according to the email we received:
all_words_spoken= mutate(final, average=mean(tokens_CHI))

#2.
all_words_mean= final %>% 
  group_by(id) %>%
  summarise(mean(tokens_CHI))

all_words= merge(final, all_words_mean)

#3.
#It keeps the big variance in the data, the analysis will be more precise.
